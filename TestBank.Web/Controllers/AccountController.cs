    ...
    public class AccountController : BaseController
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ApiKey = TestBankApiProxy.Authenticate(new Credentials() { User = model.UserName, Password = model.Password });
                    if (ApiKey != null)
                    {
                        //string userData = "ApplicationSpecific data for this user.";
                        var identity = new CustomTestBankIdentity(new UserModel() { Id =  model.UserName });
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    var newUser = TestBankApiProxy.Post<UserModel>(ApiKey, user, ResourceEndPoint.User_Post);

                    if (newUser != null && !string.IsNullOrWhiteSpace(newUser.Id))
                    {
                        return RedirectToAction("RegisterStatus", "Account");
                    }
                    /*var apiKey = TestBankApiProxy.Post<Guid>(new Credentials() { User = model.UserName, Password = model.Password }, "/auth");
                    if (apiKey != null)
                    {
                        ...
                        //string userData = "ApplicationSpecific data for this user.";
                        var identity = new CustomTestBankIdentity(new User() { UserId = model.UserName });
                        Thread.CurrentPrincipal = new GenericPrincipal(identity, new string[] { "" });
